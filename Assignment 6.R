...
#Gj??r s??nn at vi bare har de tre f??rste siffrene under "Ansvar"
DS$Ansvar <- substr(DS$Ansvar, 1, 3)

#I koden over ble verdiene omgjort til chr, s?? vi m?? endre tilbake til num
...
df <- data.frame(Product=gl(3,10,labels=c("A","B", "C")), 
                 ...
                 Sales=1:30)
#Lager en funksjon slik at summen av de observasjonen i samme ?? blir 100
fn <- function(x) (x*100)/sum(x)

#Endrer p?? dette i datasette
df <- plyr :: ddply(df, "Year", transform, Share=fn(Sales))

#Lager en plot som viser salg per ??r per selskap
ggplot() +
